...
import { ArrowBigLeft, CheckCircle, Printer } from 'lucide-vue-next'
  ...
  <div class="min-h-screen bg-gray-50">
    <nav class="bg-white/80 border-b border-gray-200">
      ...
      >
        ...
        <div>
          <span class="text-xs py-2 px-3 text-purple-800 bg-purple-100 rounded-full">{{ election.status }}</span>
        </div>
      </div>
    </nav>
    ...
    <div class="max-w-6xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
      ...
      <div class="bg-white rounded-lg shadow p-6 text-center">
        ...
        <p class="text-gray-600 mb-4" v-if="election.results_published">Official results have been published.</p>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
          <div>
            <span class="text-gray-500">Election Period:</span>
            <span class="font-medium block">{{ fmt12h(election.start_date) }} - {{ fmt12h(election.end_date) }}</span>
          </div>
          <div>
          ...
          <div>
            ...
            <span class="font-medium block">{{ (election.voter_turnout_percentage || 0).toFixed(1) }}%</span>
          </div>
          <div>
        ...
        <div class="mt-4 flex items-center justify-center gap-2">
          <button v-if="election.can_review_results && !election.results_published" @click="publish" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-md">Publish Results</button>
          <button v-if="election.can_archive" @click="archive" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-md">Archive</button>
          <button @click="printPage" class="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-800 rounded-md inline-flex items-center gap-2"><Printer class="w-4 h-4"/> Print / Save PDF</button>
        </div>
      </div>
      <!-- Results by Position -->
      <div class="space-y-6 mt-5">
        <div v-if="positions.length === 0" class="bg-white rounded-lg shadow p-6 text-center text-gray-500">
        ...
        <div v-for="pos in positions" :key="pos.id" class="bg-white rounded-lg shadow">
          ...
          <div class="p-6 space-y-4">
            <div v-if="pos.candidates?.length === 0" class="text-sm text-gray-500">No candidates.</div>
            <div v-else>
              <div v-for="(c, idx) in pos.candidates" :key="c.id"
                   :class="['border rounded-lg p-4', idx === 0 ? 'border-green-300 bg-green-50' : 'border-gray-200']">
                <div class="flex justify-between">
                  <div class="flex items-center space-x-4">
                    <div class="w-8 h-8" />
                    <div class="w-12 h-12 rounded-full bg-gray-100 overflow-hidden flex items-center justify-center">
                      <img v-if="c.profile_picture_url" :src="c.profile_picture_url" alt="" class="w-12 h-12 object-cover" />
                      <span v-else class="text-primary-600 font-medium">{{ (c.name||'')[0] }}</span>
                    </div>
                    <div>
                ...
                <div class="mt-4">
                  <div class="w-full bg-gray-200 rounded-full h-2">
                    <div class="h-2 rounded-full bg-green-600" :style="{ width: `${c.vote_percentage}%` }"></div>
                  </div>
                </div>
              </div>
              <div v-if="pos.yes_count !== undefined && pos.no_count !== undefined" class="mt-2 text-sm text-gray-700">
                YES: <span class="font-medium text-green-700">{{ pos.yes_count }}</span> · NO: <span class="font-medium text-red-700">{{ pos.no_count }}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="bg-gray-900 rounded-lg shadow text-white p-8 mt-5">
        <div class="text-center">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div v-for="pos in positions" :key="pos.id" class="bg-white/10 backdrop-blur-sm rounded-lg p-4">
              <h3 class="font-semibold mb-2">{{ pos.title }}</h3>
              <p class="text-lg font-bold">{{ pos.candidates?.[0]?.name || '—' }}</p>
              <p class="text-sm opacity-90">{{ pos.candidates?.[0]?.vote_count || 0 }} votes ({{ pos.candidates?.[0]?.vote_percentage || 0 }}%)</p>
            </div>
          </div>
        </div>
      </div>
...
<style>
@media print {
  nav, .router-link, .router-link-active, button { display: none !important; }
